...
do
    ...
    if [ ! -d "$ZIPTARGET$DIR" ]
    then
	echo "Creating directory '$ZIPTARGET$DIR'"
	mkdir -p "$ZIPTARGET$DIR"
    fi

    if [ ! -d "$ZIMTARGET$DIR" ]
    then
	echo "Creating directory '$ZIMTARGET$DIR'"
	...
	then
	    ...
	    cd `dirname "$SCRIPT"`
	    $SCRIPT --filePath="$TMP$ZIPFILE" --zimPath="$SOURCE$DIR/$ZIMFILE" --tmpDirectory="$TMP" --type=portable
