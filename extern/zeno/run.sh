...
wget --continue http://www.tntnet.org/download/tntnet-1.6.3.tar.gz
tar -xvzf tntnet-1.6.3.tar.gz
mv tntnet-1.6.3 tntnet

rm -rf cxxtools-1.4.8.tar.gz cxxtools
wget --continue http://www.tntnet.org/download/cxxtools-1.4.8.tar.gz
tar -xvzf cxxtools-1.4.8.tar.gz
mv  cxxtools-1.4.8 cxxtools

rm -rf tntdb-1.0.1.tar.gz tntdb
wget --continue http://www.tntnet.org/download/tntdb-1.0.1.tar.gz
tar -xvzf tntdb-1.0.1.tar.gz
mv tntdb-1.0.1 tntdb

svn co https://tntzenoreader.svn.sourceforge.net/svnroot/tntzenoreader/trunk tntzenoreader
...
for PROJ in `echo "cxxtools tntnet tntdb tntzenoreader"` ; do
  ...
  if [ "$PROJ" = "tntdb" ]; then
    ...
    ./configure --without-postgresql --without-mysql --without-oracle --without-doxygen 
  else
    ./configure
  fi
